...
> [!IMPORTANT]
> 当前上传的仓库不包含车辆数据字典 `data/VehicleData.sldd`。该文件是项目初始化和模型运行的必需依赖；使用前请从项目维护者处获取与当前版本匹配的文件，并放置到上述路径，否则 `initProject()` 将因缺少数据字典而终止。`setupProject()` 只能生成包含基础 Bus 定义和占位参数的数据字典，不能替代已配置的车辆数据文件。
