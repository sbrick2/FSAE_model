classdef FSAESimulationApp < handle
    ...
    methods (Access = private)
        ...
        function updateTrackPreview(app, result)
            ...
            if ~isempty(first)
                isTrackClosed = false;
                if isfield(result, "Track") && isfield(result.Track, "IsClosed") && ...
                        isscalar(result.Track.IsClosed) && logical(result.Track.IsClosed)
                    isTrackClosed = true;
                elseif isfield(result, "Meta") && isfield(result.Meta, "TrackIsClosed") && ...
                        isscalar(result.Meta.TrackIsClosed) && logical(result.Meta.TrackIsClosed)
                    isTrackClosed = true;
                elseif isprop(app, "CurrentScenario") && isstruct(app.CurrentScenario) && ...
                        isfield(app.CurrentScenario, "Track") && ...
                        isfield(app.CurrentScenario.Track, "IsClosed") && ...
                        isscalar(app.CurrentScenario.Track.IsClosed) && ...
                        logical(app.CurrentScenario.Track.IsClosed)
                    isTrackClosed = true;
                end
                ...
                if isTrackClosed
                    startX = x(first);
                    startY = y(first);
                    if isfield(result, "Track") && isfield(result.Track, "X") && ...
                            isfield(result.Track, "Y") && ~isempty(result.Track.X)
                        startX = double(result.Track.X(1));
                        startY = double(result.Track.Y(1));
                    elseif isprop(app, "CurrentScenario") && isstruct(app.CurrentScenario) && ...
                            isfield(app.CurrentScenario, "Track") && ...
                            isfield(app.CurrentScenario.Track, "X") && ...
                            isfield(app.CurrentScenario.Track, "Y") && ...
                            ~isempty(app.CurrentScenario.Track.X)
                        startX = double(app.CurrentScenario.Track.X(1));
                        startY = double(app.CurrentScenario.Track.Y(1));
                    end
                    plot(app.TrackAxes, startX, startY, 'o', ...
                        "MarkerSize", 7, ...
                        "MarkerFaceColor", [0.00, 0.65, 0.20], ...
                        "MarkerEdgeColor", [0.00, 0.35, 0.10]);
                    plot(app.TrackAxes, startX, startY, 's', ...
                        "MarkerSize", 10, ...
